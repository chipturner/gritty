...
The client probes for liveness on idle and declares the link dead after 60s with no server frame. It then enters a reconnect loop with exponential backoff (1s..10s) and a per-attempt handshake+Attach budget of 15s -- generous enough to absorb cellular RTT plus a retransmit. A keystroke while disconnected forces an immediate attempt, and for tunnel sessions also nudges the supervisor: the client touches a `connect-{name}.kick` file that the supervisor's respawn backoff wait consumes, so user presence cuts short an ssh respawn backoff (up to 60s) as well as the client's own sleep.
