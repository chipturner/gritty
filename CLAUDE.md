...
- **`server::run()`** -- takes `(client_rx, metadata_slot, SessionConfig { agent_socket_path, svc_socket_path, session_id, session_name, command, ring_buffer_cap, oauth_tunnel_idle_timeout, initial_cols, initial_rows, cwd, initial_device_id, idle_evict_timeout })`. `SessionConfig` has `Default` (1MB ring, 5s oauth timeout, 90s idle-evict). Called by e2e tests + daemon; update both.
