...
- `gritty new-session` — creates a persistent session and auto-attaches (requires running server). Alias: `new`. `-A`/`--forward-agent` forwards local SSH agent.
- `gritty new-session -t <name>` — creates a named session and auto-attaches
- `gritty attach -t <id|name>` — attaches to a session, detaches other clients. Alias: `a`. `-A`/`--forward-agent` forwards local SSH agent.
...
```bash
cargo build
cargo test                           # all tests (123 total)
...
cargo test --test e2e_test           # e2e session tests (18)
                                     # + 21 connect unit tests + 17 escape processor tests + 7 lockfile tests in lib
...
- **`protocol`** — `Frame` enum with session relay types (Data/Resize/Exit/Detached/Ping/Pong/Env/AgentForward/AgentOpen/AgentData/AgentClose), control request types (NewSession/Attach/ListSessions/KillSession/KillServer), and control response types (SessionCreated/SessionInfo/Ok/Error). `SessionEntry` struct carries per-session metadata (id, name, pty_path, shell_pid, created_at, attached, last_heartbeat). Custom tokio-util `Encoder`/`Decoder`. Wire format: `[type: u8][length: u32 BE][payload]`. Session relay: `0x01` Data, `0x02` Resize, `0x03` Exit, `0x04` Detached, `0x05` Ping, `0x06` Pong, `0x07` Env, `0x08` AgentForward, `0x09` AgentOpen, `0x0A` AgentData, `0x0B` AgentClose. Control requests: `0x10` NewSession, `0x11` Attach, `0x12` ListSessions, `0x13` KillSession, `0x14` KillServer. Control responses: `0x20` SessionCreated, `0x21` SessionInfo, `0x22` Ok, `0x23` Error.

- **`daemon`** — Listens on a single socket (`$XDG_RUNTIME_DIR/gritty/ctl.sock` or `/tmp/gritty-$UID/ctl.sock`). Manages sessions in a `HashMap<u32, SessionState>` where `SessionState` holds `JoinHandle` + `Arc<OnceLock<SessionMetadata>>` + `mpsc::UnboundedSender` for client handoff + optional name. Auto-incrementing `next_id` counter. Session resolution: name match first, then numeric id parse. Handles `NewSession` (allocate id, compute `agent-{id}.sock` path, create channel, spawn server with agent path, send SessionCreated, hand off framed connection), `Attach` (resolve session, send Ok, hand off), `ListSessions`, `KillSession`, `KillServer`. Reaps finished sessions before each operation. Signal handling: SIGTERM/SIGINT trigger clean shutdown (abort sessions, remove socket + PID file). PID file written to `socket_dir()/daemon.pid` after socket bind.

- **`server`** — `SessionMetadata` struct with pty_path, shell_pid, created_at, `AtomicBool` attached flag, `AtomicU64` last_heartbeat. `ManagedChild` wraps `tokio::process::Child` with process-group cleanup (`killpg(SIGHUP)` on drop). `run()` takes `mpsc::UnboundedReceiver<Framed<UnixStream, FrameCodec>>` + `Arc<OnceLock<SessionMetadata>>` + `PathBuf` (agent socket path). Deferred shell spawn: allocates PTY early, binds agent socket, waits for first client, reads optional `Env` frame (100ms timeout), then spawns login shell (`-l`) with `CWD=$HOME`, forwarded env vars, and `SSH_AUTH_SOCK` pointing to the agent socket. Agent forwarding: spawns an acceptor task on the agent socket; when a process connects, assigns a channel_id and sends `AgentOpen` to the client (if forwarding enabled); data relayed via `AgentData`; `AgentClose` on either side tears down the channel. On client disconnect/takeover, all agent channels are cleared and forwarding is reset. Receives clients via channel (no per-session socket). Inner relay select includes `client_rx.recv()` for client takeover — new client gets the session, old client receives `Detached` frame. Replies `Pong` to `Ping` and updates `last_heartbeat` timestamp.
...
- **`client`** — `NonBlockGuard` saves/restores stdin's `O_NONBLOCK` flag on drop (prevents breaking parent shell). `RawModeGuard` saves/restores terminal mode. `EscapeProcessor` implements SSH-style `~` escape sequences (detach/suspend/help). `run()` takes session id/name, initial framed connection, redraw flag, `ctl_path` for reconnect, `env_vars: Vec<(String, String)>`, `no_escape: bool`, and `forward_agent: bool`. On first relay, sends `Env` frame (if non-empty), then `AgentForward` (if `forward_agent` and `SSH_AUTH_SOCK` set), then `Resize`; on reconnect sends `AgentForward` + `Resize` (env_vars cleared). Agent forwarding: on `AgentOpen`, connects to local `SSH_AUTH_SOCK` and spawns bidirectional relay tasks; `AgentData` routed via HashMap; `AgentClose` tears down channels. All agent channels dropped on disconnect. Sends `Ping` every 5s; if no `Pong` within 15s, treats connection as dead. On disconnect/timeout, auto-reconnects via `ctl_path` (connect, Attach, resume relay). Prints `[reconnecting...]` / `[reconnected]` during the loop. Ctrl-C (0x03 in raw mode) exits during reconnect. Handles `Detached` frame with `[detached]` message (no reconnect on detach).
...
- **Self-backgrounding connect**: `connect` command forks via `daemonize()` (same pattern as `gritty server`), sets up SSH tunnel, signals readiness to parent (which prints socket path and exits), then manages the tunnel in the background. Socket named by hostname (`connect-{host}.sock`), overridable via `-n <name>`. `--foreground` keeps it in the foreground. Uses `flock()` lockfiles for race-free liveness detection. Four files per tunnel in socket dir: `.sock`, `.pid`, `.lock` (flock), `.dest` (destination string). Tunnel monitor respawns SSH on transient failure (exit 255); non-transient (auth, config) bails. `ConnectGuard` Drop ensures cleanup (kill SSH, remove files, release lock) on all exit paths. `setsid()` during daemonize makes the connect process the session+group leader; orphaned SSH children cleaned via `killpg()` on stale detection.
...
Full CLI with tmux-like ergonomics. Single-socket architecture. Self-daemonizing server with `--foreground` option. PID file and clean signal handling (SIGTERM/SIGINT). Ping/Pong heartbeat with auto-reconnect. Login shell with client environment forwarding. SSH agent forwarding (`--forward-agent` / `-A`). SSH-style escape sequences (`~.` detach, `~^Z` suspend, `~?` help). Self-backgrounding SSH connect with lockfile-based liveness (`gritty connect` forks, prints socket path, returns; `gritty disconnect` tears down; `gritty tunnels` lists status). All modules implemented and tested (136 tests: 17 escape processor + 28 connect + 49 protocol codec + 21 e2e session + 21 daemon integration).
...
- **`daemonize()` function** — in `main.rs` (private). Uses `nix::unistd::pipe()` + `fork()` + `setsid()`. Takes `on_ready: impl FnOnce(Pid)` callback run in parent after readiness byte received. Child returns `OwnedFd` (write end) which is passed to `daemon::run()` or `connect::run()` as `ready_fd`. Must fork before tokio runtime (no threads). Uses `nix::unistd::pipe()` (POSIX-portable), not `pipe2()` (Linux-only). Used by both `gritty server` and `gritty connect`.
...
- **`client::run()` signature** — takes `session: &str` + `Framed<UnixStream, FrameCodec>` + `redraw: bool` + `ctl_path: &Path` + `env_vars: Vec<(String, String)>` + `no_escape: bool` + `forward_agent: bool`. Called from `new_session()` (redraw=false, env_vars=[TERM,LANG,COLORTERM], no_escape/forward_agent from CLI) and `attach()` (redraw=!no_redraw, env_vars=[], no_escape/forward_agent from CLI) in main.rs. The `ctl_path` enables auto-reconnect.
- **`server::run()` signature** — takes `mpsc::UnboundedReceiver<Framed<UnixStream, FrameCodec>>` + `Arc<OnceLock<SessionMetadata>>` + `PathBuf` (agent socket path). Called directly by e2e tests (via `UnixStream::pair()` + channel) and spawned by daemon. Changing its signature requires updating both.
...
- **`daemonize()` function** — generalized with `on_ready: impl FnOnce(Pid)` callback. Server prints PID; connect prints socket path + usage hints. Same pipe mechanism: parent reads one byte from child.
