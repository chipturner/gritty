...
Nine modules behind a lib crate (`src/lib.rs` hosts `collect_env_vars()`, `spawn_channel_relay()`, `handshake()`) with thin binary entry (`src/main.rs`):
...
`Attach`: `[session_len: u16][session][client_name_len: u16][client_name][force: u8][no_replay: u8][cols: u16][rows: u16][attach_token: u64]`. Server enforces: if attached and `!force`, returns `AlreadyAttached` error; if the client presents a non-zero `attach_token` that doesn't match the server's current owner, returns `OwnerChanged` (even under `force=true`) so a stale reconnect can't steal back a session that was legitimately taken over. `no_replay` = existence probe only (daemon replies `Ok` without session handoff, no token). `cols`/`rows` are the client's current terminal size, applied to the PTY before reconnect replay so regenerated prompts and TUI repaints use the right winsize (0 = unknown). `attach_token` is the owner token from a prior successful `AttachAck`/`NewSession`; `0` means "no prior knowledge".

`SessionCreated`: `[id: u32]`. Immediately followed by `AttachAck` on the same framed connection -- the creator auto-attaches and stores the owner token for subsequent reconnects.

`AttachAck`: `[token: u64][session_id: u32]`. Reply to a successful `Attach` (or auto-attach after `NewSession`). The client stores the token and presents it on every reconnect; a silent reconnect from the current owner is returned the same token, any other acceptance (fresh attach, force takeover) rotates to a fresh token so stale clients see `OwnerChanged`. The `session_id` lets the client use the authoritative numeric id (even when the user passed `-` or a name) for subsequent reconnect/tail/fwd-socket operations, avoiding client-side races resolving `-` via `ListSessions`.
...
- **Owner token (`attach_token`)**: Every session has an `AtomicU64` owner token in `SessionMetadata`, minted by `daemon::mint_attach_token()` (a monotonic counter scoped to the daemon lifetime). `NewSession` mints the creator's token and returns it via `SessionCreated` + a trailing `AttachAck`; subsequent successful `Attach` calls reply with `AttachAck { token, session_id }` and rotate the token except for silent reconnects from the current owner (same token in, same token out -- keeps clients stable if an AttachAck is lost in flight). Reconnect attempts carry the stored token in `Frame::Attach.attach_token`; a non-zero token that doesn't match the current owner is rejected with `ErrorCode::OwnerChanged`, which the client treats as terminal. This prevents a flaky-cellular laptop A from silently stealing back a session that laptop B legitimately force-took-over while A's link was broken. Tokens are ephemeral per daemon lifetime: server restart is detected separately via `server_id` in `HelloAck`.
...
- **`server::run()`** -- takes `(client_rx, metadata, agent_path, svc_path, session_id, session_name, command, ring_buffer_cap, oauth_tunnel_idle_timeout, initial_cols, initial_rows, cwd, initial_attach_token)`. Called by e2e tests + daemon; update both.
- **`client::run()`** -- takes `(session, session_id, framed, ctl_path, env_vars, no_escape, forward_agent, forward_open, oauth_redirect, oauth_timeout, heartbeat_interval, heartbeat_timeout, client_name, expected_server_id, attach_token)`. The `attach_token` is the owner token stored across reconnects.
