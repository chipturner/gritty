...
- **Socat**: required; a missing socat fails the suite (`require_socat`). `GRITTY_SOCAT_TEST=0` makes those tests fail with an explicit message instead -- to run the rest of the suite without socat use `cargo nextest run -E 'not (binary(socat_tunnel_test) | binary(socat_bridge_test))'`. Real-sshd coverage lives in the container suite (`just test-container`), gated on Docker.
