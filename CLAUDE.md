...
- **`server::run()`** -- takes `(client_rx, metadata, agent_path, svc_path, session_id, session_name, command, ring_buffer_cap, oauth_tunnel_idle_timeout, initial_cols, initial_rows, cwd, initial_device_id)`. Called by e2e tests + daemon; update both.
- **`client::run()`** -- takes `(session, session_id, framed, ctl_path, env_vars, no_escape, forward_agent, forward_open, oauth_redirect, oauth_timeout, heartbeat_interval, heartbeat_timeout, client_name, expected_server_id, device_id)`. The `device_id` is the persistent machine identifier from `get_or_create_device_id()`.
