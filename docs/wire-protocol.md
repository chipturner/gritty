...
`SvcRequest` (svc socket dispatch, 1-byte discriminator): `OpenUrl=1`, `Send=2`, `Receive=3`, `Clipboard=5`. OpenUrl sub-protocol: `[url]\n`; server replies one byte -- `0x01` forwarded to the attached client, `0x00` not delivered (client has URL forwarding off, *or* the session is detached -- both arms reply, so `gritty open` and the `$BROWSER` shim never wait out their read timeout). Clipboard sub-protocol: `[0x01][data]` = copy (client half-closes its write side; server replies one byte -- `0x01` delivered to an attached clipboard-capable client, `0x00` dropped, `0x02` refused because `data` exceeds `CLIPBOARD_MAX_BYTES` (512 KiB; the server reads one byte past the limit and refuses rather than truncating, and `gritty copy` checks the same constant before sending so its error can quote the size) -- so `gritty copy` fails loudly instead of exiting 0 on a silent drop or a cut-down paste; an older server sends nothing and the client degrades to a soft warning; an older client -- which took any non-`0x00` reply as success -- still fails, because the server closes without draining and the client's write hits EPIPE; that is why the server must not drain before refusing), `[0x02]` = paste (server responds with clipboard content).

File transfer manifest (svc socket, not Frame protocol): sender writes `[file_count: u32][per file: [name_len: u16][name: bytes][size: u64][mode: u32]]`. Server writes one byte back to the sender: `0x01` go (`TRANSFER_GO_PAIRED`); or, immediately before closing the sender's stream, `0x02` superseded (a newer sender's manifest displaced this one in the same session) or `0x03` busy (a relay is already running) -- these let `gritty send` say why an offer ended; a receiver gets no such byte (its stream starts with the file count, so an unpaired receiver just sees EOF). On pairing the server writes `[file_count: u32]` to the receiver, then relays per-file headers `[name_len: u16][name: bytes][size: u64][mode: u32]` to the receiver, each followed by `size` bytes of data. Sentinel `[name_len: 0x0000]` ends transfer. `-` (stdin) spools to a temp file for size discovery.
