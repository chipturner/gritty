...
- **Port forwarding** -- `gritty rf 8080` to quick-check a remote web server locally, `gritty lf 5432` to let the session reach local postgres (the target defaults to your attached session; name one with `gritty rf devbox:work 8080`); survives reconnects like everything else; client-initiated only (a compromised server cannot open forwards)
