...
| `gritty tail [host:session]` | `t` | Read-only stream of session output (`^C` stops; keystrokes are ignored with a bell). Ends with `session exited [with code N]` when the session does, exiting with that code. Like every command except `connect`, it does not start a server or tunnel that is down -- the error names what is down and the `connect` command that brings it up |
...
- `-` (`send`): read data from stdin -- a receiver in file mode saves it as `stdin-YYYYMMDD-HHMMSS` (sender's local time; `send` prints the exact name up front); (`receive`): write data to stdout. `receive` with no destination also auto-switches to stdout when its stdout is redirected (e.g. `gritty receive > foo` or piped); pass a directory to force file mode
- `--timeout <seconds>`: deadline for pairing with a receiver/sender (default 300; `--no-timeout` waits indefinitely). An offer can also end before that because every session it went to closed it: a newer `send` in the same session replaces an earlier one, and a session with a transfer in progress takes no new offers -- `send` reports which of these happened per session
...
Completions are generated by the binary at shell startup, so they always match the installed version, and they complete gritty's live vocabulary as well as its flags: host names (`local`, tunnels, `[host.*]` config entries) wherever a host is expected, tunnel names for `tunnel-destroy`, and -- once you have typed the colon -- the sessions on that host for `connect`, `tail`, `rename`, `kill-session` (which also offers `local`'s sessions bare), `lf`/`rf` and `--session`. Session lookup asks the daemon directly with a 300 ms budget and never starts anything, so a host that is down simply completes nothing.

```
# Bash (~/.bashrc)
source <(COMPLETE=bash gritty)

# Zsh (~/.zshrc)
source <(COMPLETE=zsh gritty)

# Fish (~/.config/fish/completions/gritty.fish)
COMPLETE=fish gritty | source

# Elvish (~/.elvish/rc.elv)
eval (E:COMPLETE=elvish gritty | slurp)
```

PowerShell: `$env:COMPLETE = "powershell"; gritty | Out-String | Invoke-Expression; Remove-Item Env:\COMPLETE` in `$PROFILE`. `COMPLETE=` (empty) or `COMPLETE=0` disables the hook.
