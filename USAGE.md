...
- `-` (`send`): read data from stdin -- a receiver in file mode saves it as a file literally named `stdin` (`send` says so up front); (`receive`): write data to stdout. `receive` with no destination also auto-switches to stdout when its stdout is redirected (e.g. `gritty receive > foo` or piped); pass a directory to force file mode
